local mod = assert(hsw_workbench)
...
do
  ...
  function ic:find_recipe(workbench_info, tool_info, item_stacks)
    assert(workbench_info, "expected workbench_info")
    ...
    if recipe_index then
      ...
      if recipe_index then
        ...
        for name, recipe in pairs(recipe_index.recipes) do
          ...
          if recipe:matches_workbench(workbench_info) and
