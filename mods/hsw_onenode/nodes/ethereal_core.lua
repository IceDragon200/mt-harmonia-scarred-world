...
local function on_punch(pos, node, puncher, pointed_thing)
  ...
  if wielded_item_stack then
    ...
    if Groups.has_group(itemdef, "core_ethereal_material") then
  ...
  if not leftover then
    ...
    local max_len = #mod.core_ethereal_materials
    if mat_index >= max_len then
      mat_index = 0
    end
    mat_index = mat_index + 1
    meta:set_int("mat_index", mat_index)
    local mat = mod.core_ethereal_materials[mat_index]
    if mat then
...
mod:register_node("ethereal_core", {
  description = mod.S("ETHEREAL CORE"),

  codex_entry_id = mod:make_name("ethereal_core"),

  groups = {
    onenode_ethereal_core = 1,
    cracky = nokore.dig_class("wme"),
  },

  drawtype = "normal",
  tiles = {
    "hsw_core_ethereal.png",
  },
